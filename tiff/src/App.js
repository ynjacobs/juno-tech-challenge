...
import log from './assets/tiff.png'; 
...
const getIndicatorByCountry = async ( page = 1) => {  
  ...
  render() {
    ...
    return (
      <div>
      <h1 className="title"><img className="log"  src={log} alt="tiff logo"/> movies of the year  </h1>
      <div className="outer">
      {
        this.state.movies.map((movie, i) => {
          return (
            
            <div className="movie" onClick={this.handleClick.bind(this, movie.id)} key={i}>{movie.title} <img  src={logo} alt="movie icon"/>
